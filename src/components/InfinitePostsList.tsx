...
export function InfinitePostsList({posts, isError, isLoading, fetchNewPosts,
     hasMore = false }: InfinitePostsListProps){
 if(isLoading) return <h1><LoadingSpinner /> </h1>
 if(isError) return <h1>Error...</h1>
 if(posts == null || posts?.length === 0) {
return <h2 className="my-4 text-center text-2xl text-gray-500">No Posts Found</h2>
 }
     ...
     <Link href={`/profiles/${user.id}`}> <ProfileImage src={user.image}/> </Link>
        <div className="flex flex-grow flex-col">
            <div className="flex gap-1">
                <Link href={`/profiles/${user.id}`} className="font-bold focus-visible:underline outline-none hover:underline">
                {user.name}
                </Link>
            ...
            <HeartButton onClick={handleToggleLike} isLoading={toggleLike.isLoading}
